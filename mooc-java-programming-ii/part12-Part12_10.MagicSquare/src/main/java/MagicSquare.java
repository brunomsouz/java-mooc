...
public class MagicSquare {
    ...
    public ArrayList<Integer> sumsOfDiagonals() {
        ...
        for (int i = 0; i < square.length; i++) {
            firstDiag += square[i][i];
        }
        
        int secondDiag = 0;
        
        for (int i = square.length - 1, j = square.length - 1; i >= 0; i--) {
            secondDiag += square[i][Math.abs(i - j)];
        }
        
        sumsOfDiagonals.add(firstDiag);
        sumsOfDiagonals.add(secondDiag);
